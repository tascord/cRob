...
module.exports = {
    ...
    loadModules: function(client, clientConfig) {
        ...
        const se = require('./extentions/serverExtentions');
        ...
        for(var i in modulesContents) {
            ...
            if(fs.lstatSync(`./modules/${modulesContents[i]}`).isDirectory()) {
                ...
                try{var moduleData = JSON.parse(fs.readFileSync(`./modules/${modulesContents[i]}/${modulesContents[i]}.json`).toString()); } catch (err) {warn(`Module '${modulesContents[i]}' dosen't have a valid json file. Skipping load. (${err.message})`); continue;}
            
                for(var j in moduleData) {

                    var moduleConfig = moduleData[j];
                    ...
                    try {
                        if(moduleConfig.type == "command") {
                            ...
                            var module = require(`../../modules/${modulesContents[i]}/${moduleConfig.target}`);
                            module.load(moduleConfig, clientConfig, client, {clientExtentions: ce, serverExtentions: se});
                        }
